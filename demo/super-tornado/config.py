...
BaseSupervisorConf = {'program': {'user': 'root', 'autorestart': 'true', 'redirect_stderr': 'true', 'loglevel': 'info'}}
...
def make_supervisor_conf_file(project_path, appname, post, numprocs):
    ...
    supervisor_conf['group:{}s'.format(appname)] = {'programs': programs}
    for i in range(numprocs):
        supervisor_conf['program:{}-{}'.format(appname, i)] = {'command': 'python3 {}/main.py --port={}'.format(project_path, post + i), 'stdout_logfile': '{}/{}{}.log'.format(sp_log_dir, appname, i)}
